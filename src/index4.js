...
class Store {
  ...
  @action bar(){
      this.string="@action修改后字符串"
      this.number="@action修改后数字"
  }
  @action.bound barbound(){
    this.string="action.bound修改后字符串"
    this.number="action.bound修改后数字"
}
}
...
reaction(()=>[store.string,store.number],arr=>{
    console.log(`action监听,store.string:${store.string},store.number:${store.number}数组里边的值没修改一次,触发一次`)
})
// store.bar()
